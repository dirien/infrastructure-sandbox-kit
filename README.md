...
sbx run --kit ghcr.io/dirien/infrastructure-sandbox-kit:v0.10.1 infrastructure-sandbox .
```
...
The image is stamped with both its version tag (`:v0.10.1`) and `:latest`.

```bash
make load                          # build + load into sbx (as :v0.10.1 and :latest)
#   leaner (no cloud CLIs):  make load INSTALL_CLOUDS=0
#   add .NET (Pulumi C#):    make load INSTALL_DOTNET=1

sbx secret set -g pulumi           # one-time: bind your Pulumi token (docs/credentials.md)

make run                           # sbx run --template infrastructure-sandbox:v0.10.1 --kit ./kit claude .
```

Or push to a registry (as `:v0.10.1` and `:latest`) and point `--template` at it:

```bash
make build push IMAGE=ghcr.io/dirien/infrastructure-sandbox        # + VERSION=v0.10.1 to override
...
`kit/spec.yaml`, which is pinned to an immutable release tag (`v0.10.1`), not a
