...
sbx run --kit ghcr.io/dirien/infrastructure-sandbox-kit:v0.10.5 infrastructure-sandbox .
```
...
make run                           # sbx run --template infrastructure-sandbox:v0.10.5 --kit ./kit claude .
```

Or push to a registry (as `:v0.10.5` and `:latest`) and point `--template` at it:
...
```bash
sbx run --kit ghcr.io/dirien/infrastructure-kit:v0.10.5 --skills=off claude .
```
