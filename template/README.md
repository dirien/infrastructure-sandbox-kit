...
```bash
# from the repo root
make build                         # -> infrastructure-sandbox:v0.10.4 and :latest
...
DOCKER_BUILDKIT=1 docker build -f template/Dockerfile \
  -t infrastructure-sandbox:v0.10.4 -t infrastructure-sandbox:latest .
```
...
sbx run --template infrastructure-sandbox:v0.10.4 --kit ./kit claude .
```

Or publish to a registry (pushes `:v0.10.4` and `:latest`) and reference it there:
