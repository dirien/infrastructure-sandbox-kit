...
```bash
# from the repo root
make build                         # -> infrastructure-sandbox:v0.10.1 and :latest
...
DOCKER_BUILDKIT=1 docker build -f template/Dockerfile \
  -t infrastructure-sandbox:v0.10.1 -t infrastructure-sandbox:latest .
```
...
| `ISK_APM_SETUP_REPO` / `ISK_APM_SETUP_REF` | `dirien/my-claude-apm-setup` / `v0.6.7` | APM setup |
...
sbx run --template infrastructure-sandbox:v0.10.1 --kit ./kit claude .
```

Or publish to a registry (pushes `:v0.10.1` and `:latest`) and reference it there:

```bash
make build push IMAGE=ghcr.io/dirien/infrastructure-sandbox
sbx run --template ghcr.io/dirien/infrastructure-sandbox:v0.10.1 --kit ./kit claude .
```
