...
```bash
# from the repo root
make build                         # -> infrastructure-sandbox:v0.9.0 and :latest
...
DOCKER_BUILDKIT=1 docker build -f template/Dockerfile \
  -t infrastructure-sandbox:v0.9.0 -t infrastructure-sandbox:latest .
```
...
| `ISK_APM_SETUP_REPO` / `ISK_APM_SETUP_REF` | `dirien/my-claude-apm-setup` / `v0.6.5` | APM setup |
...
sbx run --template infrastructure-sandbox:v0.9.0 --kit ./kit claude .
```

Or publish to a registry (pushes `:v0.9.0` and `:latest`) and reference it there:
