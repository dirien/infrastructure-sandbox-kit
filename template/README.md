...
```bash
# from the repo root
make build                         # -> infrastructure-sandbox:v0.10.5 and :latest
...
DOCKER_BUILDKIT=1 docker build -f template/Dockerfile \
  -t infrastructure-sandbox:v0.10.5 -t infrastructure-sandbox:latest .
```
...
| `ISK_APM_SETUP_REPO` / `ISK_APM_SETUP_REF` | `dirien/my-claude-apm-setup` / `v0.6.9` | APM setup |
| `ISK_APM_VERSION` | `0.31.0` | APM CLI (pinned + SHA256; bump deliberately) |
...
sbx run --template infrastructure-sandbox:v0.10.5 --kit ./kit claude .
```

Or publish to a registry (pushes `:v0.10.5` and `:latest`) and reference it there:

```bash
make build push IMAGE=ghcr.io/dirien/infrastructure-sandbox
sbx run --template ghcr.io/dirien/infrastructure-sandbox:v0.10.5 --kit ./kit claude .
```
...
- `pulumi` in `/opt/pulumi` (symlinked onto `PATH`, with the bundled
  `pulumi-language-*` and `pulumi-resource-*` plugins), `terraform`, `tofu`,
  `kubectl`, `helm`, and, unless `INSTALL_CLOUDS=0`, `aws`, `az` and `gcloud`.
