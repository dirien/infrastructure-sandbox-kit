...
# by the binary being on PATH (idempotent guards skip what's already there), and any
# CLI still missing is retried by the kit's commands.startup step on the next start.
# Safe to run repeatedly; it converges. Set ISK_INSTALL_CLOUDS=0 to skip clouds.
set -euo pipefail
...
# --- AWS CLI v2 (pinned, SHA256-verified) ----------------------------------
...
install_aws() {
  if have aws && aws --version 2>&1 | grep -q "aws-cli/${AWSCLI_VERSION}"; then
    log "aws-cli ${AWSCLI_VERSION} already installed — skipping"; return 0
  fi
  local a; a="$(aws_arch)"                 # x86_64 | aarch64
...
# whatever is still missing. Subshells isolate `set -e` so one failure never aborts
# the others (or provisioning).
( set -e; install_aws )    || warn "aws-cli install failed (will retry on next start)"
( set -e; install_azure )  || warn "azure-cli install failed (will retry on next start)"
( set -e; install_gcloud ) || warn "gcloud install failed (will retry on next start)"

missing=""
have aws    || missing="$missing aws"
have az     || missing="$missing az"
have gcloud || missing="$missing gcloud"
if [ -n "$missing" ]; then
  warn "cloud CLIs still missing:${missing} — the kit's startup step will retry them"
else
  log "all cloud CLIs present"
fi
exit 0
