...
VERSION     ?= $(shell git describe --tags --abbrev=0 2>/dev/null || echo v0.10.0)
...
BUILD_ARGS = \
  ...
  --build-arg ISK_KUBECTL_VERSION=$(KUBECTL_VERSION) \
  --build-arg ISK_HELM_VERSION=$(HELM_VERSION) \
