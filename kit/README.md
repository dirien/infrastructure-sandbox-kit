...
It carries the network allow-list, the Pulumi Cloud credential, the agent context
— and, when the tools aren't already baked into a [template image](../template),
it provisions them (Pulumi/Terraform/OpenTofu + AWS/Azure/gcloud + APM) at
sandbox-create time.
...
- **`permissions.network.allow`** — Pulumi/Terraform/OpenTofu download + registry
  hosts, the AWS/Azure/Google installer + apt hosts, GitHub, language registries
  (npm/PyPI/Go proxy), the APM installer, the MCP hosts, and a **cloud
  control-plane starter set** (AWS STS, Azure ARM/login, `*.googleapis.com`).
  **Add the rest of your cloud/region endpoints here** — see
...
- **`credentials[].pulumi`** — injects `PULUMI_ACCESS_TOKEN` as
  ...
  the container sees only `proxy-managed`). Cloud creds are supplied separately —
...
- **`setup.install`** — runs once, as the `agent` user, before Claude launches.
  ...
  this repo at `KIT_REF`) and runs `provision.sh`, a no-op when already provisioned.
- **`agentInstructions.content`** — IaC guidance, surfaced to Claude via the
  mixin's sbx-generated `kits-memory/infrastructure.md` progressive-disclosure file.
...
- **Skip the cloud CLIs** (faster create): set `ISK_INSTALL_CLOUDS="0"`.
- **Enable .NET** (Pulumi C#): uncomment the `.NET` domains under
  ...
  path, `make build INSTALL_DOTNET=1` instead.
